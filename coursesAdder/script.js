...
const firebaseConfig = {
  ...
  document.getElementById("problemForm").addEventListener("submit", async function (event) {
    ...
    try {
      // Check if a problem with the same id, title, and order already exists
      const querySnapshot = await db.collection("problems")
        .where("id", "==", id)
        .where("title", "==", title)
      ...
      if (!querySnapshot.empty) {
        alert(
          "A problem with the same ID, title, and order already exists. Please use unique values."
        );
        return;
      }
